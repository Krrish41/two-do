...
create table if not exists public.folders (
  ...
  created_by uuid references public.authorized_users(id),
...
create table if not exists public.tasks (
  ...
  created_by uuid references public.authorized_users(id),
  assigned_to uuid references public.authorized_users(id),
  created_at timestamptz default now(),
...
-- 7. Grant standard role permissions on public schema so RLS can govern access
...
-- 9. Enable Supabase Realtime Publication for collaborative sync
do $$
begin
